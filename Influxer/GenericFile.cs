            ...
            else if (!settings.GenericFile.HeaderMissing)
            {
                ...
                if (settings.GenericFile.ColumnLayout.Count > 0)
                {
                    foreach (var c in columns)
                    {
                        ...
                        {
                            Logger.LogLine (LogLevel.Info, "Column Mismatch: Column[%0] defined in configuration %1, found %2. Cannot proceed!!", c.ColumnIndex, settings.GenericFile.ColumnLayout[c.ColumnIndex].NameInFile, c.ColumnHeader);
                            Logger.LogLine (LogLevel.Error, "Column Mismatch: Column[%0] defined in configuration %1, found %2. Cannot proceed!!", c.ColumnIndex, settings.GenericFile.ColumnLayout[c.ColumnIndex].NameInFile, c.ColumnHeader);
                            return new ProcessStatus () { ExitCode = ExitCode.InvalidArgument };
                        }
                ...
                else
                {
                    foreach (var c in columns)
                    {
                        ...
                        settings.GenericFile.ColumnLayout[c.ColumnIndex] = c.Config;
                        ColumnHeaders.Add (c);
                    }
                }
            }
